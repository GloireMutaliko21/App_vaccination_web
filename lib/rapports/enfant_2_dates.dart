...
class RapportEnfants2Dates extends StatefulWidget {
  const RapportEnfants2Dates({Key? key}) : super(key: key);

  @override
  _RapportEnfants2DatesState createState() => _RapportEnfants2DatesState();
}

class _RapportEnfants2DatesState extends State<RapportEnfants2Dates> {
  List list = <dynamic>[];

  Future<void> getEnfant() async {
    await DataSource.getInstance.getCurrentData(params: {
      "event": "ENFANTS_2_DATES",
      "idHopital": MyPreferences.idHopital,
      "date1": "${ControllersDates.date1.text}",
      "date2": "${ControllersDates.date2.text}"
    }).then((data) {
      if (mounted)
        setState(() {
          list = data;
    ...
    pdf.addPage(pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Center(
            ...
            pw.Text(
                "ENFANTS VACCINES ENTRE ${ControllersDates.date1.text} ET ${ControllersDates.date2.text}",
